...
dst = np.array([ 0.13359,-0.29557, -0.00070 ,-0.00054,0.00000 ],dtype=np.float64)


@cython.boundscheck(False)
@cython.cdivision(True)
@cython.wraparound(False)
cdef void tablegenlaser(long hmin,long hmax,double m1, double c1, double m2, double c2, double thresh):
    cdef:
...
@cython.cdivision(True)
@cython.wraparound(False)
cdef void tablegenblob(long hmin,long hmax,long smin, long smax, long v, long vmax):
    cdef:
        #unsigned char[:,:,::1] table = np.zeros((256,256,256),dtype = np.uint8)
        #unsigned char* table_ptr = &table[0,0,0]
        long x0,y0,z0
    ...
    for x0 in range(256):
        for y0 in range(256):
            for z0 in range(256):
                ...
                if chroma != 0:
                    hue = int(30 * abs(K +((g-b)/(chroma))))
                    if (hue>= hmin) & (hue <= hmax) & (int(r) >=v) & (int(r) <= vmax) & (saturation >= smin) & (saturation <= smax):
                        tablelut_ptr[(256*256*x0) + (256*y0)+z0] = 1
    #return None
...
def render_laser(long hmin,long hmax,double m1, double c1, double m2, double c2, double thresh):
    tablegenlaser(hmin,hmax,m1,c1,m2,c2,thresh)
    #tablegen(40,80,-1.0,100.0,-0.651162,242.7906,40.0) #consider changing thresh to >40

def render_blob(long hmin,long hmax,long smin, long smax, long v, long vmax):
    tablegenblob(hmin,hmax,smin,smax,v,xmax) 
...
def final_return(rect,Qinv):
    ...
    #print "res is",np.asarray(res)
    ppos = np.array([[res[0]],[res[1]],[1]])
    #print "ppos is",np.asarray(ppos) 
